...
<div class="row">
  <div class="col-xl-12">
    <div class="breadcrumb-holder">
      <h1 class="main-title float-left">{{ __('session.session_management') }}</h1>
      <ol class="breadcrumb float-right">
        <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">{{ __('default.dashboard') }}</a></li>
        <li class="breadcrumb-item active">{{ __('session.session') }}</li>
      </ol>
...
<div class="row">
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-md-6">
            <h2><i class="fa fa-bars"></i>&nbsp;{{ __('session.session') }}</h2>
          </div>
      ...
      <div class="card-body">
        ...
        <div class="">
          ...
          @endphp
          {{-- @include('backend.partials.page_numbering_seial_pick') --}}


            <div class="container-fluid">
              <div class="row">

                @include('backend.partials.page_numbering', ['route' => 'admin.session.index'])

                <div class="col-sm-12 col-md-6 pull-right">
                  <form class="form-horizontal" action="{{ route('admin.session.index') }}" method="get">
            ...
            <table width="100%" id="" class="table table-bordered table-hover display">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="action">
                    <button data-toggle="modal" data-target="#edit_page{{ $session->id }}" class="btn btn-outline-primary"><i class="fa fa-edit"></i></button>

                    {{-- edit Modal --}}
                    <div class="modal fade" id="edit_page{{ $session->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">{{ __('session.update_session') }}</h5>
                          ...
                          <div class="modal-body">
                            <form action="{{ route('admin.session.update', $session->id) }}" method="POST">
                              @csrf
                              <input type="hidden" name="old_title" value="{{ $session->title }}">
                              <div class="form-group">
                                <label class="col-form-label" for="title">{{ __('default.title') }}<span class="text-danger">*</span></label>
                                <div>
                                  <input type="text" class="form-control" name="title" value="{{ $session->title }}" id="title" required>
                                </div>
                              </div>
                              <div class="form-group">
                                ...
                                <div>
                                  <select class="form-control" name="status" id="status" required>
                                    ...
                                    <option value="0" {{ $session->status == 0 ? 'selected' : '' }}>Deactive</option>
                                  </select>
                                </div>
                              </div>
                    ...
                    <button class="btn btn-outline-danger" onClick="deleteMethod({{ json_encode($session->id) }})" role="button"><i class="fa fa-trash"></i></button>
                  </td>
                </tr>
                @endforeach
              </tbody>
            ...
            @include('backend.partials.pagination', ['table' => $sessions])
  ...
  <div class="modal fade" id="add_new" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">{{ __('session.add_session') }}</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
